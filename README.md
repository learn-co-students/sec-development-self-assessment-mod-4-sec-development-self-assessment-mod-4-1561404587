...
4. The backend for this code challenge is hosted on heroku at: https://bayside-student-lister.herokuapp.com/, and the specific endpoints available for use are listed below in the deliverables section.
...
5. On submit, persist the updated student information to the API using a PATCH request to https://bayside-student-lister.herokuapp.com/api/v1/students/:id. The updated student information should also be reflected on the front end. (When using `fetch` to make a PATCH request, make sure you capitalize the `{method: 'PATCH'}`)
